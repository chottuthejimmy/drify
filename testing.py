...
def is_spock(hand_landmarks):
    ...
    # Check for Spock sign: Thumb with pinky, index with middle, ring finger slightly apart
    # You may need to adjust these conditions based on testing
    if (abs(index_tip.x - middle_tip.x) < 0.02 and
        abs(index_tip.y - middle_tip.y) < 0.02 and
        abs(thumb_tip.x - pinky_tip.x) > 0.04):
        return True
    return False
